...
export const assistantOutputSchema = z.object({
  ...
  recommendation: z.enum(["beclean", "becalm", "besoft"]),
  safety: z.boolean(),
});
...
const productLinks = {
  beclean: {
    name: "BeClean",
    link: "https://rebkabeauty-beazeepb.manus.space/produto/beclean",
    reason: "é o primeiro passo prático para começar uma rotina simples.",
  },
  becalm: {
    name: "BeCalm",
    link: "https://rebkabeauty-beazeepb.manus.space/produto/becalm",
    reason: "entra depois da limpeza como uma etapa de cuidado com textura leve.",
  },
  besoft: {
    name: "BeSoft",
    link: "https://rebkabeauty-beazeepb.manus.space/produto/besoft",
    reason: "é a etapa de hidratação confortável para finalizar a rotina.",
  },
} as const;

const safetyTerms = /alerg|irrita|ardor|ardend|vermelhid|incha[cç]|dor|ferida|les[aã]o|dermatite|eczema|ros[aá]cea|acne|espinha|gr[aá]vid|gesta|amament|medicamento|rem[eé]dio|tratamento|p[oó]s.?procedimento/i;

export const isSafetyQuestion = (content: string) => safetyTerms.test(content);

export function inferRecommendedProduct(content: string): "beclean" | "becalm" | "besoft" {
...
Retorne SOMENTE JSON válido com as chaves answer, recommendation e safety. recommendation deve ser beclean, becalm ou besoft. safety deve ser true quando a orientação exige cautela de saúde. answer não deve conter preço, link, recomendação final nem markdown.`;
